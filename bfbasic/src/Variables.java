...
/**
 * The main job of this class is to figure out the ordering of variables and
 * arrays that results in the smallest file size.
 * 
...
public class Variables {
	...
	/**
	 * interactions between variables (the order that the variables occur in the
	 * source code). Allows testing the efficiency of a new ordering
	 */
	private int[] _interaction;

	/**
	 * Prevents the first "x" elements to be moved. This is to prevent the internal
	 * variables from moving apart from each other, specifically for hardcoded BF
	 * code blocks (such as the string generator).
	 */
	private int _first;

	public Variables(String[] names, Integer[] cellCount, LinkedList<String> interaction, int first) {
	...
	/**
	 * @return memory positions of each variable, in the order given by getOrder().
	 */
	public int[] getStart() {
		return _start;
	}
	...
	/**
	 * Calls makeBestMove until it returns false, meaning it has found the best
	 * arrangement it knows about.
	 */
	...
	/**
	 * @return Determines the cost in >>> <<< arrows of the current variable
	 *         ordering.
	 */
	...
	/**
	 * Attempts to move each variable right one at a time, testing to see which
	 * movement helps the most. Note that since a move right is like a swap, the
	 * same variable position is moved right again to restore the original ordering.
	 * If a better ordering has been found then the variables are left in that
	 * order.
	 * 
