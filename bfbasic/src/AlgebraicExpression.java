...
public class AlgebraicExpression {
	...
	public String getToken() {
		...
		if (_ae.equals("   ")) { // empty
			...
			_ae = _ae.substring(1);
			/*
			 * } else if (p == '!') { // operator level 4: negate tokentype = TOKEN_LEVEL4;
			 * token = p + ""; ae = ae.substring(1);
			 */
	...
	public String parse() {
		...
		Stack<Integer> tokentypes = new Stack<>();
		Stack<Integer> operands = new Stack<>();
		Stack<Integer> operandTypes = new Stack<>();
		...
		try {
			while (_tokentype != TOKEN_NULL) {
				if (_tokentype == TOKEN_LPAREN) {
					tokens.push(_token);
					tokentypes.push(_tokentype);
					functions.push(_token);
				} else if (_tokentype == TOKEN_FUNCTION) {
					tokens.push(_token);
					tokentypes.push(TOKEN_LPAREN);
					functions.push(_token);
				} else if (_tokentype == TOKEN_RPAREN) {
					while ((tokentypes.peek()).intValue() != TOKEN_LPAREN) {
						operator = tokens.pop();
						operand2 = operands.pop();
						int simple = operandTypes.pop();
						operand1 = operands.peek();
						int operand1Type = operandTypes.peek();
						if (operand1Type >= 0) {
							if (bfbasic._debug > 0) {
						...
						/*
						 * if ((tokentypes.pop()).intValue() == TOKEN_LEVEL4) { operand1 = -1; } else {
						 * operand1 = operands.peek(); }
						 */
					...
					if (!functions.peek().equals("")) {
						String funct = functions.peek().toUpperCase();
						if (funct.equals("NOT")) {
							// V1=NOT(V1)
							// @T0 ([-]) - T0=255
							...
							parsed += "@_0-@_T" + (_tempvar - 1) + "[@_0-@_T" + (_tempvar - 1) + "-]@_0[@_T"
									+ (_tempvar - 1) + "+@_0-]";
						} else {
							// V1=V2(V1)
							...
							if (bfbasic._debug > 0) {
								parsed += "\n(_T" + (_tempvar - 1) + "=" + funct + "(_T" + (_tempvar - 1) + "))\n";
							}

							// @V1 = @_T" + (_tempvar - 1)
							// @V2 = "@~" + funct
							parsed += "@_T" + (_tempvar - 1) + "[@~" + funct + ">>+<<@_T" + (_tempvar - 1) + "-]@~"
									+ funct + ">>[[>>]+[<<]>>-]+[>>]<[<[<<]>+<" + "@_T" + (_tempvar - 1) + "+@~" + funct
									+ ">>[>>]<-]<[<<]>[>[>>]<+<[<<]>-]>[>>]<<[-<<]";
					...
					operands.push(_tempvar);
					operandTypes.push(val); // means simple
					_tempvar++;
					...
					if (_token.equalsIgnoreCase("INKEY")) {
						...
						parsed += "@_RH[@_0+@_RH-]@_RL[@_1+@_RL-]@_3+++++++[@_2+++++++++++@_3-]"
								...
								+ "@_2[@_RL+@_2-]+@_1[@_2-@_1[-]]@_2[@_RH+@_2-]@_0-]" + "++++++[@_RH+++++++++@_0-]"
						...
						// @T0[@V2+@T0-] V2=T0:T0=0
						parsed += "@_T" + _tempvar + "[-]@" + _token + "[@_T" + _tempvar + "+@_0+@" + _token + "-]@_0[@"
								+ _token + "+@_0-]";
					}
					operands.push(_tempvar);
					operandTypes.push(-1);
					...
					while (!tokens.empty()) {
						if ((tokentypes.peek()).intValue() == TOKEN_LPAREN) {
							break;
						}
						if (_tokentype > (tokentypes.peek()).intValue()) {
							break;
						}
						operator = tokens.pop();
						operand2 = operands.pop();
		...
		if (operandTypes.size() > 0) {
			int value = operandTypes.pop();
			if (value >= 0) {
				parsed += "@_T" + operands.pop().intValue() + "[-]" + addTo(value);
			}
		}
		_parse = parsed;
		return parsed;
	...
	String getCode(String operand1, String operator_, String operand2, int simple) {
		...
		if (bfbasic._debug > 0) {
			if (operand1.equals("_T-1")) {
				...
				code += "\n(" + operand1 + "=" + operand1 + bfbasic.debugtext(operator_) + operand2 + ")\n";
			} else {
				code += "\n(" + operand1 + "=" + operand1 + bfbasic.debugtext(operator_) + simple + ")\n";
			}
		}
		...
		if (operator.equals("!")) {
			...
			// @T0[@V1+@T0-] V1=T0:T0=0
			if (simple < 0) {
				code += "@_0[-]@_T" + operand2 + "[@_0-@_T" + operand2 + "-]" + "@_0[@_T" + operand2 + "+@_0-]";
			} else {
				return null;
			}
			...
			// @T0[@V2+@T0-] V2=T0:T0=0
			if (simple < 0) {
				code += "@" + operand2 + "[@" + operand1 + "+@_0+@" + operand2 + "-]" + "@_0[@" + operand2 + "+@_0-]";
			} else {
			...
			// @T1-] T1=T1-1:LOOP
			if (simple < 0) {
				code += "@" + operand1 + "[@_1+@" + operand1 + "-]@_1[" + "@" + operand2 + "[@" + operand1 + "+@_0+@"
						+ operand2 + "-]" + "@_0[@" + operand2 + "+@_0-]@_1-]";
			} else {
				return null;
			}
		} else if (operator.equals("/")) {
			...
			// @T3] LOOP:(T3=0)
			if (simple < 0) {
				code += "@" + operand1 + "[@_3+@" + operand1 + "-]@_3[" + "@" + operand2 + "[@_2+@_0+@" + operand2
						+ "-]@_0[@" + operand2 + "+@_0-]" + "@_2[@_3-[@_1+@_0+@_3-]@_0[@_3+@_0-]+@_1[@_0-@_1[-]]"
			...
			// @T1[-]] END IF:T1=0
			if (simple < 0) {
				code += "@" + operand1 + "[@_1+@" + operand1 + "-]-@" + operand2 + "[@_1-@_0+@" + operand2 + "-]@_0[@"
						+ operand2 + "+@_0-]@_1[@" + operand1 + "+@_1[-]]";
			} else {
				return null;
			}
		} else if (operator.equals("<>")) {
			...
			// @T4[-] T4=0
			if (simple < 0) {
				code += "@" + operand1 + "[@_4+@" + operand1 + "-]" + "@" + operand2 + "[@_1+@_2+@_0+@" + operand2
						+ "-]" + "@_0[@" + operand2 + "+@_0-]@_2[@_4[@_3+@_0+@_4-]@_0[@_4+@_0-]+"
						+ "@_3[@_1-@_4-@_0-@_3[-]]@_0[@_2[-]+@_0-]@_2-]" + "@_1[@" + operand1 + "-@_1[-]]@_4[-]";
			} else {
				return null;
			}
		} else if (operator.equals("<=")) {
			...
			if (simple < 0) {
				code += "@" + operand1 + "[@_4+@" + operand1 + "-]-" + "@" + operand2 + "[@_1+@_2+@_0+@" + operand2
						+ "-]" + "@_0[@" + operand2 + "+@_0-]@_2[@_4[@_3+@_0+@_4-]@_0[@_4+@_0-]+"
						+ "@_3[@_1-@_4-@_0-@_3[-]]@_0[@_2[-]+@_0-]@_2-]" + "@_4[@" + operand1 + "+@_4[-]]@_1[-]";
			} else {
				return null;
			}
		} else if (operator.equals(">")) {
			...
			// @T0[-]] END IF:T0=0
			if (simple < 0) {
				code += "@" + operand1 + "[@_0+@" + operand1 + "-]@_0[@" + operand1 + "-@_0[-]]" + "@" + operand2
						+ "[@_0+@_1+@" + operand2 + "-]" + "@_1[@" + operand2 + "+@_1-]@_0[@" + operand1
						+ "[-]-@_0[-]]";
			} else {
				return null;
			}
			...
			// ] END IF
			if (simple < 0) {
				code += "@" + operand1 + "[@_0+@" + operand1 + "-]@_0[@_0[-]" + "@" + operand2 + "[@_0+@_1+@" + operand2
