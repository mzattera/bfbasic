...
public class bfbasic {
	public static final String VERSION = "1.50 rc3";

	static final int WRAP_DEFAULT = 72;

	static boolean _insert = false, _crlf = false, _needPre = true, _needPost = false, _vartable = false;
	/**
	...
	static LinkedHashMap<String, Integer> _var = new LinkedHashMap<>(), _label = new LinkedHashMap<>();
	static int _line = 0, _wrapWidth = 0, _optLevel = 2;
	static String _p = "", _sourceLine = "", _curfile = "";
	static char _a;
	static Stack<Integer> _doStack = new Stack<>(), _forStack = new Stack<>(), _ifStack = new Stack<>(),
			_caseDepthStack = new Stack<>();
	static Stack<String> _forTopStack = new Stack<>(), _forStepStack = new Stack<>(), _caseStack = new Stack<>(),
			_libraryStack = new Stack<>();
	static int _annex = 1, _doAnnex = 1, _ifAnnex = 1, _forAnnex = 1, _gosubAnnex = 0, _selectAnnex = 1;
	...
	public static void main(String[] cmdline) {
		...
		System.out.println("BFBASIC Basic Compiler, version " + VERSION + ".  Copyright 2001-2005 Jeffry Johnston.");
		...
		for (n = 0;; n++) {
			try {
				if (cmdline[n].equals("-?") || cmdline[n].equals("--?") || cmdline[n].equals("/?")
						|| cmdline[n].equals("--help") || cmdline[n].equals("-h")) { // help
					usage();
					System.exit(0);
					...
					_debug = 1;
				} else if (cmdline[n].equals("-dd") || cmdline[n].equals("-D")) { // verbose debug output
					_wrapWidth = WRAP_DEFAULT;
					_debug = 2;
		...
		while (!_libraryStack.empty()) {
			_sourceLine = _libraryStack.pop().trim();
			parse();
			bf_include();
		}
		...
		if (_debug == 3 | _vartable == true) {
			Iterator<String> iter = _var.keySet().iterator();
			while (iter.hasNext()) {
				String key = iter.next();
				int next = _var.get(key);
				write("$" + key + "=" + next);
			}
			write("\n");
		}
	...
	public static String arrows(String o, boolean write) {
		...
		LinkedList<String> interaction = new LinkedList<>();
		...
		for (int l = 0; l < o.length(); l++) {
			...
			if (c == '@') {
				...
				if (write) {
					...
					if (_debug != 3) {
						if (mp < pos) {
							out.append(string(pos - mp, '>'));
						}
						if (mp > pos) {
							out.append(string(mp - pos, '<'));
						}
					}
					mp = pos;
					if (write && _debug > 1) {
		...
		if (!write) {
			if (_optLevel == 1) {
				...
				String[] names = (String[]) _var.keySet().toArray(new String[_var.keySet().size()]);
				Variables var = new Variables(names,
						(Integer[]) _var.values().toArray(new Integer[_var.values().size()]), interaction, 10);
				var.findBestOrder();
				...
				for (int i = 0; i < start.length; i++) {
					_var.put(names[Math.abs(order[i])], start[i]);
				}
			}
		}
		return out.toString();
	...
	public static void bf_color() {
		...
		String expr = (_sourceLine.substring(0, n)).trim();
		_sourceLine = "PRINT \"\033[\"; " + expr + "; \"m\";:" + (_sourceLine.substring(n)).trim();
		if (_debug > 0) {
	...
	public static void bf_do() {
		...
		if (parseif()) {
			...
			if (_p.equals("WHILE")) {
				...
				_doStack.push(-_doAnnex);
				_sourceLine = "IF " + expr + " THEN GOTO _D" + (_doAnnex + 1) + ":" + (_sourceLine.substring(n)).trim();
			} else {
				errout("Syntax error");
			}
		}
	...
	public static void bf_end() {
		...
		if (_p.equalsIgnoreCase("IF")) {
			...
			if (_p.equalsIgnoreCase("SELECT")) {
				...
				if (_caseStack.empty()) {
					errout("END SELECT without SELECT");
				}
				int ltmp = _caseDepthStack.pop();
				int l = ltmp;
				_sourceLine = "";
				for (int i = 1; i <= l; i++) {
	...
	public static void bf_exit() {
		...
		if (_p.equals("DO")) {
			...
			if (_doStack.empty()) {
				errout("EXIT DO without DO");
			}
			int l = _doStack.pop();
			_sourceLine = "GOTO _D" + (Math.abs(l) + 1) + ":" + _sourceLine;
			if (l > 0) {
				l = -l;
			}
			_doStack.push(l);
			...
			if (_caseStack.empty()) {
				errout("EXIT SELECT without SELECT");
			}
	...
	public static void bf_for() {
		...
		_forStack.push(-_forAnnex);
		_forTopStack.push(new String(expr3));
		_forStepStack.push(new String(s));
		_sourceLine = expr + "=" + expr2 + ":" + p0 + ":" + (_sourceLine.substring(n)).trim();
		if (_debug > 0) {
			writeTemp("\n{FOR " + debugtext(expr) + "=" + debugtext(expr2) + " TO " + debugtext(expr3) + "}\n");
		}
		_forAnnex += 2;
	}
	...
	public static void bf_gosub() {
		...
		int n = _gosubAnnex++;
		if (n == 256) {
			System.out.println("Warning: 257th GOSUB encountered.  " + "Will crash if using 8-bit cells.");
		}
		_sourceLine = "_GS(_GP)=" + n + ":_GP=_GP+1:GOTO " + _p + ":" + ":_G" + n + ":" + _sourceLine;
		if (_debug > 0) {
			writeTemp("\n{GOSUB " + _p + "}\n");
		}
	}
	...
	public static void bf_if() {
		...
		if (_p.equals("GOTO")) {
			...
			if (_p.equals("DO")) {
				...
				_doStack.push(l);
				if (_debug > 0) {
					o = "\n(IF " + debugtext(expr) + " THEN {EXIT DO}(GOTO _D" + (Math.abs(l) + 1) + "))\n";
				}
			} else if (_p.equals("FOR")) {
				...
				_forStack.push(l);
				if (_debug > 0) {
					o = "\n(IF " + debugtext(expr) + " THEN {EXIT FOR}(GOTO _F" + (Math.abs(l) + 1) + "))\n";
				}
			} else {
				errout("Syntax error");
			}
	...
	public static void bf_input() {
		...
		if (_a == '(') {
			...
			_sourceLine = (_sourceLine.substring(n)).trim();
			AlgebraicExpression ae = new AlgebraicExpression(expr);
			o = pre() + ae.parse() + "@~" + _p + ">>[-]<<@_T0[@~" + _p + ">>+<<@_T0-]" + "@_0+[-@_1,.----------"
					+ (_crlf ? "---" : "") + "[@_0+" + "@_2+++++" + (_crlf ? "" : "+") + "[@_1------"
					+ (_crlf ? "-" : "") + "@_2-]" + "@_T0[@_2+@_T0-]@_2[@_T0++++++++++@_2-]@_1[@_T0+@_1-]]@_0]"
					+ (_crlf ? "++++++++++.[-]" : "") + "@~" + _p + ">[-]<@_T0[@~" + _p + ">+<@_T0-]@~" + _p
					+ ">>[>>[-]<<-[>>+<<-]+>>]>[-]<<<[<<]>[>[>>]>+<<<[<<]>-]>-<<" + post();
			if (_debug > 0) {
			...
			o += pre() + "@" + _p + "[-]@_0+[-@_1,.----------" + (_crlf ? "---" : "") + "[@_0+" + "@_2+++++"
					+ (_crlf ? "" : "+") + "[@_1------" + (_crlf ? "-" : "") + "@_2-]" + "@" + _p + "[@_2+@" + _p
					+ "-]@_2[@" + _p + "++++++++++@_2-]" + "@_1[@" + _p + "+@_1-]]@_0]"
					+ (_crlf ? "++++++++++.[-]" : "") + post();
		}
		writeTemp(o); // arrows(o);
	}
	...
	public static void bf_locate() {
		...
		String expr2 = (_sourceLine.substring(0, n)).trim();
		_sourceLine = "PRINT \"\033[\"; " + expr + "; \";\"; " + expr2 + "; \"H\";:"
				+ (_sourceLine.substring(n)).trim();
		if (_debug > 0) {
	...
	public static void bf_loop() {
		...
		if (parseif()) {
			...
			l = _doStack.pop();
			String temp = "GOTO _D" + Math.abs(l) + ":";
			if (l < 0) {
				temp += "_D" + (-l + 1) + ":";
			}
			...
			if (_p.equals("WHILE")) {
				...
				l = _doStack.pop();
				String temp = "IF " + expr + " THEN GOTO _D" + Math.abs(l) + ":";
				if (l < 0) {
					temp += "_D" + (-l + 1) + ":";
				}
				_sourceLine = temp + _sourceLine;
				...
				l = _doStack.pop();
				String temp = "IF NOT(" + expr + ") THEN GOTO _D" + Math.abs(l) + ":";
				if (l < 0) {
					temp += "_D" + (-l + 1) + ":";
				}
				_sourceLine = temp + _sourceLine;
	...
	public static void bf_next() {
		...
		String expr = (_sourceLine.substring(0, n)).trim();
		int l = _forStack.pop();
		String m = _forTopStack.pop();
		String s = _forStepStack.pop();
		String temp = expr + "=" + expr + " + " + s + ":IF NOT(" + expr + "=" + m + " + " + s + ") THEN GOTO _F"
				+ Math.abs(l) + ":";
		if (l < 0) {
			temp += "_F" + (-l + 1) + ":";
		}
		...
		if (parseif()) {
			...
			while (!parseif()) {
				...
				if (_sourceLine.length() > 0 && _sourceLine.charAt(0) == '\"') {
					...
					int n;
					if ((n = _p.indexOf('\"')) == -1) {
						errout("Mismatched quotation mark");
					}
					if (_a == ';') {
						sameline = true;
						_a = ' ';
				...
				} else if ((_sourceLine.length() >= 4) && (_sourceLine.substring(0, 4)).equalsIgnoreCase("CHR$")) {
					// PRINT CHR$(expr)
					// (...) TEMP0=expr
				...
				} else if ((_sourceLine.length() >= 6) && (_sourceLine.substring(0, 6)).equalsIgnoreCase("SPACE$")) {
					// PRINT SPACE$(expr)
					// (@T0[-]) (T0=0)
					// (@T1[-]) (T1=0)
					// (...) TEMP0=expr
				...
				} else if ((_sourceLine.length() >= 7) && (_sourceLine.substring(0, 7)).equalsIgnoreCase("STRING$")) {
	...
	public static void bf_randomize() {
		...
		if (_p.equalsIgnoreCase("KEY")) {
			...
			// ] LOOP
			// @T0] LOOP
			String o = pre() + "@_0" + bf_text("Random seed?") + "+[@_0-@_1,----------" + (_crlf ? "---" : "")
					+ "[@_0+@_1[@_RH+@_1-]" + "@_RH[@_RL+@_1+@_RH-]@_1[@_RH+@_1-]]@_0]" + post();
			if (_debug > 0) {
				o = "\n(RANDOMIZE KEY)\n" + o;
			}
			writeTemp(o); // arrows(o);
	...
	public static void bf_swap() {
		...
		parse();
		String o = pre() + "@" + p0 + "[@_0+@" + p0 + "-]@" + _p + "[@" + p0 + "+@" + _p + "-]@_0[@" + _p + "+@_0-]"
				+ post();
		if (_debug > 0) {
			o = "\n(SWAP " + p0 + ", " + _p + ")\n" + o;
		}
	...
	public static String bf_text(String ascii) {
		...
		for (int n = 0; n < ascii.length(); n++) {
			...
			if (b < 15 || b > 126) {
				errout("Invalid ascii character: " + (int) b);
			}
			for (int c = 0; c <= 6; c++) {
		...
		for (int n = 0; n < ascii.length(); n++) {
			...
			if (mpnew < mp) {
				code += string(mp - mpnew, '<');
			}
			if (mpnew > mp) {
				code += string(mpnew - mp, '>');
			}
			mp = mpnew;

			// adjust cell value as decided above
			if (b < value[mp]) {
				code += string(value[mp] - b, '-');
			}
			if (b > value[mp]) {
				code += string(b - value[mp], '+');
			}
			value[mp] = b;
			code += ".";
		}
	...
	public static void errout(String e) {
		...
		System.out.print(e);
		if (_line > 0) {
			System.out.print(", line " + _line + " in " + _curfile);
		}
		System.out.println(".");
		System.exit(1);
	}
	...
	public static int findexpr(String endexpr, int startdepth) {
		...
		if (ee == -1 && endexpr.equals("")) {
			ee = _sourceLine.length();
		}
		return ee;
	}
	...
	public static String itoa() {
		...
		String o = "@_1[-]@_2[-]@_3[-]@_4[-]@_T0[@_3+[@_4+@_0+@_3-]@_0[@_3+@_0-]+"
				+ "@_4----------[@_0-@_4[-]]@_0[@_2+@_3[-]@_0-]" + "@_2[@_4+@_0+@_2-]@_0[@_2+@_0-]+"
				+ "@_4----------[@_0-@_4[-]]@_0[@_1+@_2[-]@_0-]" + "@_T0-]@_1[@_0++++++++[@_1++++++@_2++++++@_0-]"
				+ "@_1.@_2.[-]@_1[-]]@_2[@_0++++++++[@_2++++++@_0-]" + "@_2.[-]]@_0++++++++[@_3++++++@_0-]@_3.[-]";
		return o;
	}
	...
	public static String label(String userLabel) {
		...
		for (int n = 0; n < userLabel.length(); n++) {
			...
			if (c != '_' && (c < 'A' || c > 'Z') && (c < '0' || c > '9')) {
				errout("Label \'" + _p + "\' contains the invalid " + "character \'" + c + "\'");
			}
			if (c >= '0' && c <= '9' && n == 0) {
		...
		if (_label.containsKey(userLabel)) {
			...
			n = _label.get(userLabel);
		} else {
			// doesn't exist, create new entry
			_label.put(userLabel, _annex);
			n = _annex++;
		}
	...
	public static void last() {
		...
		if (_gosubAnnex > 0) {
			...
			for (int n = 0; n < _gosubAnnex; n++) {
				...
				o += "@_T2+@_T0[@_T1+@_T2[-]@_T0-]-@_T1[@_T0+@_T1-]@_T2[" + label("_G" + n) + "+@_G-@_T2-]";
			}
			o += "@_T0[-]" + post();
		}
	...
	public static boolean parseif() {
		boolean ans = true;
		if (_sourceLine.length() > 0 && _sourceLine.charAt(0) != '\'' && _sourceLine.charAt(0) != ':') {
			ans = false;
		}
		return ans;
	}
